...
if 'manage_users' in st.session_state.user['permissions_expanded']:
    with cols[1]:
        ...
        with tabs[0]:
            with st.form(key='create_user', border=False, enter_to_submit=False):
                ...
                if st.form_submit_button('Create User', help='Create a new user with the provided details'):
                    if 'Admin' in permissions and 'Admin' not in st.session_state.user['permissions']:
        ...
        with tabs[1]:
            ...
            if 'Admin' in [row['permissions'] for row in st.session_state.users.data if row['email'] == selected_email][0] and 'Admin' not in st.session_state.user['permissions'] and selected_email is not st.session_state.user['email']:
        ...
        with tabs[2]:
            ...
            if 'Admin' in [row['permissions'] for row in st.session_state.users.data if row['email'] == selected_email][0] and 'Admin' not in st.session_state.user['permissions'] and selected_email is not st.session_state.user['email']:
                ...
                if 'Admin' in selected_permissions and 'Admin' not in st.session_state.user['permissions']:
                    st.error('Cannot grant admin permissions')
                else:
