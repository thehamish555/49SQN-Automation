...
with cols[0]:
    st.markdown('#### Lesson Plan Viewer')
...
with cols[1]:
    tabs = st.tabs(['Auto Fill Lesson Plans', 'Lesson Plan Manager'])
    with tabs[0]:
        with st.form(key='create_lesson', border=False, enter_to_submit=False):
            ...
            if st.form_submit_button('Create Lesson Plan', use_container_width=True):
                replacements = {
        ...
        try:
            ...
            st.success(f'Lesson Plan *"{selected_file}"* Created!')
            sub_cols = st.columns(2)
            with sub_cols[0]:
                st.download_button('Download as PDF', data=pdf_bytes.getvalue(), file_name=f'{selected_file} - {date.strftime("%d-%m-%Y")}.pdf', mime='application/octet-stream', icon=':material/download:', use_container_width=True)
            with sub_cols[1]:
                if st.button('Preview', icon=':material/visibility:', use_container_width=True):
    ...
    with tabs[1]:
        if st.session_state.user['role'] == 'admin':
            with st.form(key='submit_lesson_plan', enter_to_submit=False):
                uploaded_pdf = st.file_uploader('Upload a Lesson Plan', type=['pdf'])
            ...
            with st.form(key='remove_lesson_plan'):
                ...
                if st.form_submit_button('**:red[Remove Lesson Plan]**'):
